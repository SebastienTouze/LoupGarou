    ...
    public function process()
    {
        if( $this->request->getMethod() == 'POST' )
        {
            ...
            switch ($arrayFormulaire['type']) 
            {
                case 1: //Villageois
                {
                    $userVoteContre = $this->em->getRepository('AppBundle:User')->find($arrayFormulaire['votePour']);
                    
                    $aDejaVote = $this->em->getRepository('AppBundle:Vote')->findOneBy(array('votant' => $this->userVotant->getId(), 'type' => $arrayFormulaire['type'], 'jour' => $this->jourCourant));
                ...
                case 2: //Loups garous
                {
                    $userVotePour = $this->em->getRepository('AppBundle:User')->find($arrayFormulaire['votePour']);
