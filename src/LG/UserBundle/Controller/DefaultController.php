...
class DefaultController extends Controller
{
    ...
    public function indexAction($visibility)
    {
        ...
        $mayor = 'aucun';
        if($testMayor = $em->getRepository('LGUserBundle:User')->findOneBy(array('maire' => true)) )
            $mayor = $testMayor;
        ...
        $arraySynthesisVillageVotes = $em->getRepository('LGUserBundle:Vote')->findPersoneElue(1, $dbParameters->getJour());
        $arraySynthesisMayorVotes = $em->getRepository('LGUserBundle:Vote')->findPersoneElue(3, $dbParameters->getJour());
    ...
    private function DayNightSwicher($dbParameters, $em) {
        ...
        elseif($dbParameters->getJour() == 1 && ($currentTime >= $dbParameters->getHeureNuit() || $currentTime <= $dbParameters->getHeureJour() ))
        {
            ...
            $arrayCondamned = $em->getRepository('LGUserBundle:Vote')->findPersoneElue(1, $dbParameters->getJour());
            $arrayMayor = $em->getRepository('LGUserBundle:Vote')->findPersoneElue(2, $dbParameters->getJour());
            ...
            if($idMayor)
            {
                $mayor = $em->getRepository('LGUserBundle:User')->find($idMayor);
                $mayor->setMaire(true);
                $em->persist($mayor);
            }
            else
            ...
            if($idCondamned)
            {
                $dead = $em->getRepository('LGUserBundle:User')->find($idCondamned);
                $dead->setVivant(false);
                $dead->setMaire(false);
                $em->persist($dead);
            }
